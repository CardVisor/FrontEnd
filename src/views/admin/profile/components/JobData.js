...
export default function JobData(props) {
  ...
  const [averageAgeByJobTypeAndAll, setAverageAgeByJobTypeAndAll] = useState(null);
  ...
  const [findTop3CardTypesByJobType, setFindTop3CardTypesByJobType] = useState(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([countByJobTypeAndAllResponse, averageAgeByJobTypeAndAllResponse,mostsalaryDataResponse,paymentByJobTypeAndAllResponse,topCardsByJobTypeResponse,findTop3CardTypesByJobTypeResponse]) => {

        setCountByJobTypeAndAll(countByJobTypeAndAllResponse.data.find(row => row[0] === filter)[1]);
        setAverageAgeByJobTypeAndAll(averageAgeByJobTypeAndAllResponse.data.find(row => row[0] === filter)[1]);
        setMostsalaryData(mostsalaryDataResponse.data.find(row => row[0] === filter)[1]);
        setPaymentByJobTypeAndAll(paymentByJobTypeAndAllResponse.data.find(row => row['job_type'] === filter)['avg_payment']);

        const topCardsByJobTypeData = topCardsByJobTypeResponse.data.topCardsByJobType;
        const allCards = topCardsByJobTypeResponse.data.all;
        const topCardsByJob = filter === 'all' ? allCards : topCardsByJobTypeData.find(row => row[0] === filter);

        setTopCardsByJobType(topCardsByJob ? (filter === 'all' ? topCardsByJob : topCardsByJob.slice(1)) : []);
        ...
        {loading ? (
          ...
          <div>
            ...
            <Text color={textColorPrimary} fontSize='xl'>
              ...
              평균 연봉 : {mostsalaryData}<br></br>
              평균 소비금액 : {paymentByJobTypeAndAll}<br></br>
              가장 많이 사용한 카드 : {topCardsByJobType ? topCardsByJobType.join(', ') : '데이터를 불러오는 중...'}<br></br>

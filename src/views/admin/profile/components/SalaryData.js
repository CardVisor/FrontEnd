    ...
    Promise.all([
      ...
      .then(
        ([
          ...
          setCustsalaryNum(
            custsalaryNumResponse.data.find((row) => row[0] === filter)[1]
          );
          setAgeSalaryRangeAndAll(
            ageSalaryRangeAndAllResponse.data.find(
              (row) => row[0] === filter
            )[1]
          );
          const custSalaryData = custSalaryResponse.data.find(
            (row) => row[0] === filter
          );
          setCustSalary(
            custSalaryData ? custSalaryData[1] : "데이터를 불러오는 중..."
          );
          setPaymentBySalaryAndAll(
            paymentBySalaryAndAllResponse.data.find(
              (row) => row["cust_salary"] === filter
            )["avg_payment"]
          );

          if (filter === "all") {
            setTop3CardByCustSalary(
              top3CardByCustSalaryResponse.data["all"].join(", ")
            );
          } else {
            const cardData = top3CardByCustSalaryResponse.data[
              "top3CardTypes"
            ].find((row) => row[0] === filter);
            setTop3CardByCustSalary(
              cardData
                ? cardData.slice(1).join(", ")
                : "데이터를 불러오는 중..."
            );
          }

...
export default function AgeData(props) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            axios.get(API_SERVER + '/customer/ageGroup'),
            axios.get(API_SERVER + '/customer/averageAgeGroups'),
            axios.get(API_SERVER + '/customer/topSalaryByAgeRange'),
            axios.get(API_SERVER + '/customer/averageByAgeRange'),
            axios.get(API_SERVER + '/customer/alltop3CardTypes'),
            axios.get(API_SERVER + '/customer/top3MccCodeByAgeRange'),
        ])
            .then(
                ([
                    ageGroupResponse,
                    ...
                    setAverageByAgeRange(
                        averageByAgeRangeResponse.data.find((row) => row.age_range === filter)?.pay_amount ||
                            '데이터가 없습니다.'
                    );
                    ...
                    <div>
                        ...
                        <Stack divider={<StackDivider />} spacing="3" mt="20px">
                            ...
                            <Box>
                                <Heading size="s" textTransform="uppercase">
                                    가장 많이 사용한 카드 :{' '}
                                    {topCardsAgeRange && topCardsAgeRange.map((card) => card.cardName).join(', ')}{' '}
                                </Heading>
                            </Box>
                            <Box>
                                <Heading size="s" textTransform="uppercase">

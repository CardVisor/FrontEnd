...
const [lastweeklydata, setLastWeeklyData] = useState([]);
...
//6주간 매주결제내역
    useEffect(() => {
        axios({
          method: "get",
          url: "/main/selectPerWeeklyamount",
        })
        .then((res) => {
          ...
          const weeklyname = res.data.map(item => item.week);
      ...
      useEffect(() => {
        ...
        .then((res) => {
          ...
          setLastWeeklyData(formattedNumbers);
        })
        .catch((err) => {
          console.log("Error fetching data:", err);
        });
 ...
 <>
        ...
        <Flex w='100%' flexDirection={{ base: "column", lg: "row" }}>
          ...
          {lineChartDataTotalSpent1.length > 0 && (
                <LineChart

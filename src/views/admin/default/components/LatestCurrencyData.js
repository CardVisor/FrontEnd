...
function LatestCurrencyData(props) {
    ...
    useEffect(() => {
      ...
      axios({
        ...
        .then((res) => {
          ...
          const usaCurrency = res.data.find(currency => currency.currencyCode === "USD");
          if (usaCurrency) {
  ...
  onChange={(e) => {
    ...
    const selectedCurrency = currencydata.find(currency => currency.currencyCode === selectedCurrencyCode);
    if (selectedCurrency) {
...
>
{currencydata && currencydata.map((currency, index) => (
              <option
                style={{ fontSize: '16px' }}

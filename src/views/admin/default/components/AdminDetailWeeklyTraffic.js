...
export const TrafficProvider = (props) => {
    ...
    useEffect(() => {
        axios
            .all([
                axios.get(API_SERVER + '/main/detailselectWeektransaction'),
                axios.get(API_SERVER + '/main/detailselectPerWeeklytransaction'),
            ])
            .then(
...
function AdminMonthTrafficDisplay(props) {
    const { memo, textColor, barChartDataWeekTraffic, barChartOptionsWeekTraffic, total6transaction } =
            ...
            <Box h="240px" mt="auto">
                {barChartDataWeekTraffic.length > 0 &&
                    ...
                    barChartOptionsWeekTraffic.xaxis.categories.length > 0 && (
                        <BarChart chartData={barChartDataWeekTraffic} chartOptions={barChartOptionsWeekTraffic} />
                    )}
            </Box>
        </>
    );

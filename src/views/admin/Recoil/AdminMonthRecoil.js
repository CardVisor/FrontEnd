...
const YourComponent = (props) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const [res1] = await Promise.all([axios.get(API_SERVER + '/main/selectLastYearAndPerMonthamount')]);
                    ...
                    <Box minH="260px" minW="75%" mt="auto">
                        {lineChartDataTotalSpent.length > 0 &&
                            ...
                            optionLineChart.xaxis.categories.length > 0 && (
                                <LineChart chartData={lineChartDataTotalSpent} chartOptions={optionLineChart} />
                            )}
                    </Box>
                </>
            )}

...
function TopComparePaymentSamePeriod(props) {
  ...
  useEffect(() => {
    ...
    axios({
      method: "get",
      url:
        API_SERVER + `/international/comparePaymentSamePeriod?month=${month}`,
    })
      .then((res) => {
      ...
      <InterMiniStatistics
        startContent={
          <IconBox
            ...
            bg={boxBg}
            icon={
              <Icon w="32px" h="32px" as={AiOutlineStock} color={brandColor} />
            }
          />
        }
        ...
        value={
          comparePayment &&
          `\u{20A9} ${moneyFormat(
            comparePayment.currentSum - comparePayment.lastSum
          )} 원`
        }
        comparePercentage={
          comparePayment &&
          paymentPercentage(comparePayment.lastSum, comparePayment.currentSum)
        }
      />
    </>
  );

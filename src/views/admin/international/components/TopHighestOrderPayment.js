...
function TopHighestOrderPayment(props) {
  ...
  useEffect(() => {
    axios({
      method: "get",
      url: API_SERVER + "/international/highestOrderPayment",
    })
      .then((res) => {
  ...
  useEffect(() => {
    if (paymentRank !== null) {
      const timer = setInterval(() => {
        ...
        setTimeout(() => {
          setCurrentIndex(
            (currentIndex) => (currentIndex + 1) % paymentRank.length
          );
          setIsVisible(true);
        }, 500);
      }, 3500);
      ...
      <InterMiniStatistics
        startContent={
          <IconBox
            ...
            h="56px"
            bg={boxBg}
            icon={<Icon w="32px" h="32px" as={FaChartPie} color={brandColor} />}
          />
        }
              ...
              <>
                ...
                {chartNationInfo
                  .filter(
                    (target) =>
                      target.currencyCode ===
                        paymentRank[currentIndex].CURRENCY_CODE &&
                      target.nationCode === paymentRank[currentIndex].NATION
                  )
                  .map((target) => {
